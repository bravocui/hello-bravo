...
class AIAssistantService:
    def __init__(self):
        ...
        logger.info("[INIT] Initializing AI Assistant Service with ADK")
    ...
    def _get_available_categories(self, db: Session) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"[OK] Found {len(category_names)} categories: {category_names}"
            )
    ...
    def _create_system_prompt(self, category_names: List[str]) -> str:
        ...
        logger.info(
            f"[PROMPT] Creating system prompt with categories: {category_names_str}"
        )
...
Example Good Notes of "Others" category: Sum: 'Personal' ($13.42) + 'Home' ($11.67) = $25.09.
        ...
        logger.info("[OK] System prompt created successfully")
        logger.debug(f"[INFO] System prompt length: {len(prompt)} characters")
        logger.debug(f"[INFO] System prompt preview: {prompt[:200]}...")
        return prompt

    def _initialize_agent_and_runner(self, db: Session):
        """Initialize the ADK agent and runner with database context"""
        logger.info("[AGENT] Initializing ADK agent and runner")
        category_names = self._get_available_categories(db)
        system_prompt = self._create_system_prompt(category_names)

        logger.info(f"[AGENT] Creating agent with prompt length: {len(system_prompt)}")
        ...
        logger.info(
            f"[AGENT] Agent created with instruction length: {len(self.agent.instruction)}"
        )
        ...
        logger.info(
            f"[OK] ADK Agent initialized: {self.agent.name} using model {self.agent.model}"
        )
        logger.info(f"[OK] ADK Runner initialized with app_name: {self.app_name}")
    ...
    def _process_image(self, image_file: UploadFile) -> Part:
        ...
        logger.info(
            f"[IMAGE] Processing image: {image_file.filename} ({image_file.content_type})"
        )

        if not image_file.content_type.startswith("image/"):
            logger.error(f"[ERROR] Invalid file type: {image_file.content_type}")
            raise HTTPException(
        ...
        try:
            ...
            logger.info(f"[SIZE] Original image size: {image.size}")

            # Convert to RGB if necessary
            if image.mode != "RGB":
                logger.info(f"[CONVERT] Converting image from {image.mode} to RGB")
            ...
            if max(image.size) > max_size:
                ...
                logger.info(f"[RESIZE] Resized image to: {new_size}")
            ...
            image_blob = Blob(
                data=processed_image_bytes.getvalue(), mime_type="image/jpeg"
            )
            image_part = Part.from_blob(image_blob)
            logger.info("[OK] Image processed successfully")
            return image_part
    ...
    async def process_expense_with_ai(
        ...
        logger.info(
            f"[START] Starting expense processing for user {user_id}, session {session_id}"
        )
        logger.info(
            f"[PROMPT] Prompt: {prompt[:100]}{'...' if len(prompt) > 100 else ''}"
        )
        logger.info(f"[IMAGE] Number of images: {len(images) if images else 0}")

        try:
            # Initialize agent and runner if not already done
            if not self.agent or not self.runner:
                logger.info(
                    "[AGENT] Agent or runner not initialized, creating new agent and runner"
                )
                self._initialize_agent_and_runner(db)
            else:
                logger.info("[OK] Using existing agent and runner")

            # Create session before using runner
            logger.info(
                f"[SESSION] Creating session for user {user_id}, session {session_id}"
            )
            session = await self._get_or_create_session(user_id)
            logger.info(f"[OK] Session created successfully {str(session.id)}")

            # Prepare message parts
            logger.info("[PARTS] Preparing message parts")
            message_parts = [Part(text=prompt)]

            # Add images if provided
            if images:
                logger.info(f"[IMAGE] Processing {len(images)} images")
                for i, image_file in enumerate(images):
                    logger.info(
                        f"[IMAGE] Processing image {i+1}/{len(images)}: {image_file.filename}"
                    )
            ...
            logger.info(
                f"[MESSAGE] Created user message with {len(message_parts)} parts"
            )

            # Use the runner to process the message (runner handles sessions internally)
            logger.info("[RUNNER] Using ADK runner to process message")
            try:
                ...
                logger.info("[EVENTS] Processing response events")
                for event in response_events:
                    all_events.append(event)
                    logger.info(f"[EVENTS] Received event: {type(event).__name__}")
                    ...
                    if hasattr(event, "content") and event.content:
                        if hasattr(event.content, "parts") and event.content.parts:
                            ...
                            logger.info(
                                f"[OK] Found content in event: {response_text[:100]}..."
                            )
                            break
                        else:
                        ...
                        logger.info(
                            f"[OK] Found text in event: {response_text[:100]}..."
                        )
                        break
                    elif hasattr(event, "message") and event.message:
                        if hasattr(event.message, "content") and event.message.content:
                            if (
                                hasattr(event.message.content, "parts")
                                and event.message.content.parts
                            ):
                                # Extract text from Content object
                                text_parts = []
                                ...
                                response_text = " ".join(text_parts).strip()
                                logger.info(
                                    f"[OK] Found message content: {response_text[:100]}..."
                                )
                                break
                            else:
                            ...
                            logger.info(
                                f"[OK] Found message text: {response_text[:100]}..."
                            )
                            break

                logger.info(f"[STATS] Collected {len(all_events)} events from runner")

                # If no response text found, try to extract from all events
                if not response_text and all_events:
                    logger.info(
                        "[SEARCH] No direct response found, analyzing all events"
                    )
                    for i, event in enumerate(all_events):
                        logger.info(
                            f"[EVENT] Event {i}: {type(event).__name__} - {str(event)[:100]}..."
                        )
                        ...
                        if hasattr(event, "__str__"):
                            ...
                            if event_str and event_str != "None":
                                ...
                                logger.info(
                                    f"[OK] Using event string as response: {response_text[:100]}..."
                                )
                                break
                ...
                logger.info("[OK] Runner processing completed successfully")
            ...
            logger.info(
                f"[RESPONSE] Raw response length: {len(response_text)} characters"
            )
            logger.debug(
                f"[RESPONSE] Raw response: {response_text[:200]}{'...' if len(response_text) > 200 else ''}"
            )
            ...
            try:
                ...
                if "```json" in response_text:
                    ...
                    if start != -1 and end != 0:
                        ...
                        logger.warning(
                            "[WARN] No JSON array found, using full response"
                        )
                ...
                if not isinstance(parsed_data, list):
                    logger.error("[ERROR] Response is not a list")
                    # raise ValueError("Response is not a list")

                for i, item in enumerate(parsed_data):
                    if not isinstance(item, dict):
                        logger.warning(f"[WARN] Skipping non-dict item {i}")
                        continue

                    # Validate required fields
                    if "category" not in item or "amount" not in item:
                        logger.warning(
                            f"[WARN] Skipping item {i} with missing required fields"
                        )
                        continue
                ...
                logger.info(
                    f"[STATS] Processed {len(entries)} entries with confidence {confidence}"
                )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        logger.info("[HEALTH] Checking AI assistant health status")
        status = {
        ...
        logger.info(f"[HEALTH] Health status: {status}")
        return status
